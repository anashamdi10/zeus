...
@section('title') {{ __('main.Edit_Order') }}
{{ $title }}
@endsection
...
@section('content')
    @component('components.breadcrumb')
        @slot('breadcrumb_title')
            <h3> {{ __('main.Edit_Order') }}</h3>
        @endslot
        <li class="breadcrumb-item"> {{ __('main.Edit_Order') }}</li>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <form action={{route('orders.update',$info->id)}} method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    {{method_field('PUT')}}
                    <div class="mb-3">
                        <label class="col-form-label pt-0" for="name_ar"> {{ __('main.Order_Number') }} </label>
                        <input class="form-control" id="name_ar" type="text" value="{{$info->order_number}}"   name="order_number">
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="user_id" id="categories" class="form-control" >
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}"  {{($info->user_id==$user->id)? "selected":""}}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="col-form-label pt-0" for="name_en">{{ __('main.Total') }}</label>
                        <input class="form-control" id="name_en" rows="4"  value="{{$info->total}}" name="total">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="categories" class="form-control" >
                            ...
                            <option value="approve" {{($info->status=="approve")? "selected":""}}>{{ __('main.Approve') }}</option>
                            <option value="disapprove" {{($info->status=="disapprove")? "selected":""}}> {{ __('main.Disapprove') }} </option>
                            <option value="received" {{($info->status=="received")? "selected":""}}>{{ __('main.Received') }}</option>
                            <option value="not_received" {{($info->status=="not_received")? "selected":""}}> {{ __('main.Not_Received') }}</option>
                        </select>
                    </div>
                                    ...
                                    <td>
                                        <select   name="product_id[]"  class="form-control">
                                            ...
                                            @foreach($products as $product)
                                                <option value="{{$product->id}}" {{($product->id==$value->product_id)?"selected" :""}}>{{$product->name_ar}}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    
                                    <td><input type="text" class="form-control" value="{{$value->price}}" name="price[]" required></td>
                                    <td><input type="text" class="form-control" value="{{$value->serial}}" name="serial[]" required></td>
                                    <td><input type="text" class="form-control" value="{{$value->secret}}" name="secret[]" required></td>
